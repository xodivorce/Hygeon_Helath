...
// Doctor categories
$doctor_categories = [
    "General Physician", "Cardiologist", "Neurologist", "Orthopedic Surgeon", "Pediatrician",
    "Dermatologist", "Gynecologist", "Psychiatrist", "ENT Specialist", "Ophthalmologist", "Dentist"
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_doctor'])) {
    ...
    if ($user) {
        if ($user['user_type'] == 2) {
            ...
            // Promote to doctor with selected category
            $stmt = $pdo->prepare("UPDATE user SET user_type = 2, doctor_category = :doctor_category WHERE user_email = :user_email");
            $stmt->bindParam(':user_email', $user_email);
            $stmt->bindParam(':doctor_category', $doctor_category);
            $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_doctor'])) {
    ...
    if ($stmt->rowCount() > 0) {
        // Demote doctor to patient
        $stmt = $pdo->prepare("UPDATE user SET user_type = 3, doctor_category = NULL WHERE user_email = :user_email");
        $stmt->bindParam(':user_email', $user_email);
        $stmt->execute();
...
// Fetch all doctors
$stmt = $pdo->prepare("SELECT user_id, user_name, user_email, doctor_category FROM user WHERE user_type = 2");
$stmt->execute();
...
<div class="space-y-6 p-8 bg-white rounded-lg shadow-md">
    ...
    <form action="" method="POST" class="space-y-4 bg-gray-50 p-6 rounded-lg shadow-sm">
        ...
        <div class="flex items-center space-x-4">
            <label for="doctor_category" class="w-1/4 text-gray-700 font-medium">Select Doctor Category:</label>
            <select id="doctor_category" name="doctor_category" class="w-2/4 px-4 py-2 border border-gray-300 rounded-lg" required>
                <option value="">-- Select Category --</option>
                <?php foreach ($doctor_categories as $category): ?>
        ...
        <table class="min-w-full table-auto border-collapse text-left">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="px-4 py-2"><?php echo htmlspecialchars($doctor['doctor_category']); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
